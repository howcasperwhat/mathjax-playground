...
function confirmEditState() {
  ...
  if (!playState.exists(name)) {
    ...
    playState.switchActive(name) // active
...
<template>
  <div flex="~ col gap-2" of-auto>
    ...
    >
      ...
      <button
        v-for="[name, item], idx in filtered.slice(0, length)"
        :ref="`item-${idx}`"
        :key="name" :title="name"
      ...
      <button
        ...
        text-gray bd b-op-80 op-40
        w-max btn-sm active:op-80
